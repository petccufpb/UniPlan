...
const renderInlineTokens = (tokens: Token[] | undefined): React.ReactNode => {
	...
	return tokens.map((token, index) => {
		switch (token.type) {
			case 'strong':
				return (
					<Text key={token.raw || index} style={styles.strong}>
			...
			case 'em':
				return (
					<Text key={token.raw || index} style={styles.em}>
						{renderInlineTokens(token.tokens)}
					</Text>
				)
			...
			case 'del':
				return (
					<Text key={token.raw || index} style={styles.del}>
						{renderInlineTokens(token.tokens)}
					</Text>
				)
			...
			case 'link':
				return (
					<Link key={token.href || index} src={token.href} style={styles.link}>
						{renderInlineTokens(token.tokens)}
					</Link>
				)

			case 'text':
				return <Text key={token.raw || index}>{token.raw}</Text>
...
const renderToken = (token: Token, index: number): React.ReactNode => {
	switch (token.type) {
		...
		case 'alignmentContainer':
			return (
				<View key={index} style={{ textAlign: token.align }}>
					{token.tokens?.map((childToken: Token, childIndex: number) => renderToken(childToken, childIndex))}
				</View>
			)
		case 'heading': {
			...
			return (
				<View key={index} style={styles.list}>
					{(token.items as ListItemToken[]).map((item: ListItemToken, itemIndex: number) => {
						const itemKey = item.raw || `item-${itemIndex}`

						return (
							<View key={itemKey} style={styles.listItem}>
								<Text style={styles.listItemSymbol}>{(token as ListToken).ordered ? `${itemIndex + 1}.` : '•'}</Text>

								<Text style={styles.listItemContent}>{renderInlineTokens(item.tokens)}</Text>
							</View>
						)
					})}
				</View>
